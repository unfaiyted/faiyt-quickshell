import QtQuick
import Quickshell.Io

Item {
    id: emojiResults
    visible: false

    property string typeName: "emoji"
    property int maxResults: 48

    // Comprehensive emoji database
    property var emojis: [
        // Smileys & Emotion
        { emoji: "😀", name: "grinning face", keywords: ["happy", "smile", "joy"] },
        { emoji: "😃", name: "grinning face with big eyes", keywords: ["happy", "smile", "joy"] },
        { emoji: "😄", name: "grinning face with smiling eyes", keywords: ["happy", "smile", "joy", "laugh"] },
        { emoji: "😁", name: "beaming face with smiling eyes", keywords: ["happy", "smile", "grin"] },
        { emoji: "😆", name: "grinning squinting face", keywords: ["happy", "laugh", "satisfied"] },
        { emoji: "😅", name: "grinning face with sweat", keywords: ["hot", "happy", "laugh", "relief"] },
        { emoji: "🤣", name: "rolling on the floor laughing", keywords: ["laugh", "lol", "rofl", "funny"] },
        { emoji: "😂", name: "face with tears of joy", keywords: ["laugh", "cry", "happy", "lol"] },
        { emoji: "🙂", name: "slightly smiling face", keywords: ["smile", "happy"] },
        { emoji: "🙃", name: "upside-down face", keywords: ["silly", "sarcasm"] },
        { emoji: "😉", name: "winking face", keywords: ["wink", "flirt"] },
        { emoji: "😊", name: "smiling face with smiling eyes", keywords: ["happy", "blush", "smile"] },
        { emoji: "😇", name: "smiling face with halo", keywords: ["angel", "innocent"] },
        { emoji: "🥰", name: "smiling face with hearts", keywords: ["love", "adore", "crush"] },
        { emoji: "😍", name: "smiling face with heart-eyes", keywords: ["love", "crush", "adore"] },
        { emoji: "🤩", name: "star-struck", keywords: ["wow", "excited", "amazing"] },
        { emoji: "😘", name: "face blowing a kiss", keywords: ["kiss", "love", "flirt"] },
        { emoji: "😗", name: "kissing face", keywords: ["kiss", "love"] },
        { emoji: "😚", name: "kissing face with closed eyes", keywords: ["kiss", "love"] },
        { emoji: "😙", name: "kissing face with smiling eyes", keywords: ["kiss", "love"] },
        { emoji: "🥲", name: "smiling face with tear", keywords: ["happy", "sad", "grateful"] },
        { emoji: "😋", name: "face savoring food", keywords: ["yummy", "delicious", "tongue"] },
        { emoji: "😛", name: "face with tongue", keywords: ["tongue", "playful", "silly"] },
        { emoji: "😜", name: "winking face with tongue", keywords: ["tongue", "wink", "silly", "crazy"] },
        { emoji: "🤪", name: "zany face", keywords: ["crazy", "silly", "wild"] },
        { emoji: "😝", name: "squinting face with tongue", keywords: ["tongue", "playful"] },
        { emoji: "🤑", name: "money-mouth face", keywords: ["money", "rich", "dollar"] },
        { emoji: "🤗", name: "hugging face", keywords: ["hug", "love", "care"] },
        { emoji: "🤭", name: "face with hand over mouth", keywords: ["oops", "quiet", "shy"] },
        { emoji: "🤫", name: "shushing face", keywords: ["quiet", "secret", "shh"] },
        { emoji: "🤔", name: "thinking face", keywords: ["think", "hmm", "consider"] },
        { emoji: "🤐", name: "zipper-mouth face", keywords: ["secret", "quiet", "zip"] },
        { emoji: "🤨", name: "face with raised eyebrow", keywords: ["suspicious", "skeptical"] },
        { emoji: "😐", name: "neutral face", keywords: ["meh", "blank", "indifferent"] },
        { emoji: "😑", name: "expressionless face", keywords: ["blank", "annoyed"] },
        { emoji: "😶", name: "face without mouth", keywords: ["silent", "speechless"] },
        { emoji: "😏", name: "smirking face", keywords: ["smirk", "flirt", "smug"] },
        { emoji: "😒", name: "unamused face", keywords: ["meh", "annoyed", "bored"] },
        { emoji: "🙄", name: "face with rolling eyes", keywords: ["eyeroll", "annoyed", "whatever"] },
        { emoji: "😬", name: "grimacing face", keywords: ["awkward", "nervous"] },
        { emoji: "🤥", name: "lying face", keywords: ["lie", "pinocchio"] },
        { emoji: "😌", name: "relieved face", keywords: ["relief", "calm", "peaceful"] },
        { emoji: "😔", name: "pensive face", keywords: ["sad", "thoughtful"] },
        { emoji: "😪", name: "sleepy face", keywords: ["tired", "sleep"] },
        { emoji: "🤤", name: "drooling face", keywords: ["drool", "yummy", "hungry"] },
        { emoji: "😴", name: "sleeping face", keywords: ["sleep", "zzz", "tired"] },
        { emoji: "😷", name: "face with medical mask", keywords: ["sick", "mask", "covid"] },
        { emoji: "🤒", name: "face with thermometer", keywords: ["sick", "fever", "ill"] },
        { emoji: "🤕", name: "face with head-bandage", keywords: ["hurt", "injured"] },
        { emoji: "🤢", name: "nauseated face", keywords: ["sick", "gross", "vomit"] },
        { emoji: "🤮", name: "face vomiting", keywords: ["sick", "vomit", "gross"] },
        { emoji: "🤧", name: "sneezing face", keywords: ["sneeze", "sick", "cold"] },
        { emoji: "🥵", name: "hot face", keywords: ["hot", "heat", "sweating"] },
        { emoji: "🥶", name: "cold face", keywords: ["cold", "freezing", "ice"] },
        { emoji: "🥴", name: "woozy face", keywords: ["drunk", "dizzy", "tipsy"] },
        { emoji: "😵", name: "dizzy face", keywords: ["dizzy", "dead", "knocked out"] },
        { emoji: "🤯", name: "exploding head", keywords: ["mind blown", "shocked", "wow"] },
        { emoji: "🤠", name: "cowboy hat face", keywords: ["cowboy", "western", "yeehaw"] },
        { emoji: "🥳", name: "partying face", keywords: ["party", "celebration", "birthday"] },
        { emoji: "🥸", name: "disguised face", keywords: ["disguise", "incognito"] },
        { emoji: "😎", name: "smiling face with sunglasses", keywords: ["cool", "sunglasses", "awesome"] },
        { emoji: "🤓", name: "nerd face", keywords: ["nerd", "geek", "glasses"] },
        { emoji: "🧐", name: "face with monocle", keywords: ["thinking", "curious", "monocle"] },
        { emoji: "😕", name: "confused face", keywords: ["confused", "unsure"] },
        { emoji: "😟", name: "worried face", keywords: ["worried", "concerned"] },
        { emoji: "🙁", name: "slightly frowning face", keywords: ["sad", "frown"] },
        { emoji: "😮", name: "face with open mouth", keywords: ["surprised", "wow", "shock"] },
        { emoji: "😯", name: "hushed face", keywords: ["surprised", "quiet"] },
        { emoji: "😲", name: "astonished face", keywords: ["shocked", "surprised", "wow"] },
        { emoji: "😳", name: "flushed face", keywords: ["embarrassed", "blush", "shy"] },
        { emoji: "🥺", name: "pleading face", keywords: ["puppy eyes", "please", "beg"] },
        { emoji: "😦", name: "frowning face with open mouth", keywords: ["surprised", "sad"] },
        { emoji: "😧", name: "anguished face", keywords: ["shocked", "sad"] },
        { emoji: "😨", name: "fearful face", keywords: ["scared", "fear"] },
        { emoji: "😰", name: "anxious face with sweat", keywords: ["nervous", "anxious", "worried"] },
        { emoji: "😥", name: "sad but relieved face", keywords: ["sad", "relief"] },
        { emoji: "😢", name: "crying face", keywords: ["cry", "sad", "tear"] },
        { emoji: "😭", name: "loudly crying face", keywords: ["cry", "sob", "sad"] },
        { emoji: "😱", name: "face screaming in fear", keywords: ["scream", "scared", "horror"] },
        { emoji: "😖", name: "confounded face", keywords: ["confused", "frustrated"] },
        { emoji: "😣", name: "persevering face", keywords: ["struggling", "frustrated"] },
        { emoji: "😞", name: "disappointed face", keywords: ["sad", "disappointed"] },
        { emoji: "😓", name: "downcast face with sweat", keywords: ["sad", "tired"] },
        { emoji: "😩", name: "weary face", keywords: ["tired", "frustrated"] },
        { emoji: "😫", name: "tired face", keywords: ["exhausted", "frustrated"] },
        { emoji: "🥱", name: "yawning face", keywords: ["yawn", "tired", "bored"] },
        { emoji: "😤", name: "face with steam from nose", keywords: ["angry", "frustrated", "triumph"] },
        { emoji: "😡", name: "pouting face", keywords: ["angry", "mad", "rage"] },
        { emoji: "😠", name: "angry face", keywords: ["angry", "mad"] },
        { emoji: "🤬", name: "face with symbols on mouth", keywords: ["swear", "curse", "angry"] },
        { emoji: "😈", name: "smiling face with horns", keywords: ["devil", "evil", "mischief"] },
        { emoji: "👿", name: "angry face with horns", keywords: ["devil", "angry", "evil"] },
        { emoji: "💀", name: "skull", keywords: ["dead", "death", "skeleton"] },
        { emoji: "☠️", name: "skull and crossbones", keywords: ["death", "danger", "poison"] },
        { emoji: "💩", name: "pile of poo", keywords: ["poop", "shit", "crap"] },
        { emoji: "🤡", name: "clown face", keywords: ["clown", "circus", "silly"] },
        { emoji: "👹", name: "ogre", keywords: ["monster", "demon", "scary"] },
        { emoji: "👺", name: "goblin", keywords: ["monster", "demon", "tengu"] },
        { emoji: "👻", name: "ghost", keywords: ["ghost", "halloween", "spooky", "boo"] },
        { emoji: "👽", name: "alien", keywords: ["alien", "ufo", "space", "extraterrestrial"] },
        { emoji: "👾", name: "alien monster", keywords: ["alien", "game", "space invader"] },
        { emoji: "🤖", name: "robot", keywords: ["robot", "machine", "bot", "ai"] },

        // Gestures & People
        { emoji: "👋", name: "waving hand", keywords: ["wave", "hello", "goodbye", "hi", "bye"] },
        { emoji: "🤚", name: "raised back of hand", keywords: ["hand", "stop"] },
        { emoji: "🖐️", name: "hand with fingers splayed", keywords: ["hand", "five", "high five"] },
        { emoji: "✋", name: "raised hand", keywords: ["hand", "stop", "high five"] },
        { emoji: "🖖", name: "vulcan salute", keywords: ["spock", "star trek", "live long"] },
        { emoji: "👌", name: "OK hand", keywords: ["ok", "perfect", "good"] },
        { emoji: "🤌", name: "pinched fingers", keywords: ["italian", "chef kiss", "perfection"] },
        { emoji: "🤏", name: "pinching hand", keywords: ["small", "tiny", "little"] },
        { emoji: "✌️", name: "victory hand", keywords: ["peace", "victory", "two"] },
        { emoji: "🤞", name: "crossed fingers", keywords: ["luck", "hope", "fingers crossed"] },
        { emoji: "🤟", name: "love-you gesture", keywords: ["love", "rock", "ily"] },
        { emoji: "🤘", name: "sign of the horns", keywords: ["rock", "metal", "devil"] },
        { emoji: "🤙", name: "call me hand", keywords: ["call", "shaka", "hang loose"] },
        { emoji: "👈", name: "backhand index pointing left", keywords: ["left", "point"] },
        { emoji: "👉", name: "backhand index pointing right", keywords: ["right", "point"] },
        { emoji: "👆", name: "backhand index pointing up", keywords: ["up", "point"] },
        { emoji: "👇", name: "backhand index pointing down", keywords: ["down", "point"] },
        { emoji: "☝️", name: "index pointing up", keywords: ["up", "point", "one"] },
        { emoji: "👍", name: "thumbs up", keywords: ["like", "good", "yes", "approve", "ok"] },
        { emoji: "👎", name: "thumbs down", keywords: ["dislike", "bad", "no", "disapprove"] },
        { emoji: "✊", name: "raised fist", keywords: ["fist", "power", "solidarity"] },
        { emoji: "👊", name: "oncoming fist", keywords: ["punch", "fist bump"] },
        { emoji: "🤛", name: "left-facing fist", keywords: ["fist bump", "punch"] },
        { emoji: "🤜", name: "right-facing fist", keywords: ["fist bump", "punch"] },
        { emoji: "👏", name: "clapping hands", keywords: ["clap", "applause", "bravo"] },
        { emoji: "🙌", name: "raising hands", keywords: ["celebrate", "hooray", "yay"] },
        { emoji: "👐", name: "open hands", keywords: ["hands", "jazz hands"] },
        { emoji: "🤲", name: "palms up together", keywords: ["prayer", "please"] },
        { emoji: "🤝", name: "handshake", keywords: ["handshake", "deal", "agreement"] },
        { emoji: "🙏", name: "folded hands", keywords: ["pray", "please", "thank you", "namaste"] },
        { emoji: "✍️", name: "writing hand", keywords: ["write", "pen"] },
        { emoji: "💪", name: "flexed biceps", keywords: ["strong", "muscle", "flex", "arm"] },

        // Hearts & Love
        { emoji: "❤️", name: "red heart", keywords: ["love", "heart", "red"] },
        { emoji: "🧡", name: "orange heart", keywords: ["love", "heart", "orange"] },
        { emoji: "💛", name: "yellow heart", keywords: ["love", "heart", "yellow", "friendship"] },
        { emoji: "💚", name: "green heart", keywords: ["love", "heart", "green"] },
        { emoji: "💙", name: "blue heart", keywords: ["love", "heart", "blue"] },
        { emoji: "💜", name: "purple heart", keywords: ["love", "heart", "purple"] },
        { emoji: "🖤", name: "black heart", keywords: ["love", "heart", "black", "dark"] },
        { emoji: "🤍", name: "white heart", keywords: ["love", "heart", "white", "pure"] },
        { emoji: "🤎", name: "brown heart", keywords: ["love", "heart", "brown"] },
        { emoji: "💔", name: "broken heart", keywords: ["heartbreak", "sad", "broken"] },
        { emoji: "❤️‍🔥", name: "heart on fire", keywords: ["love", "passion", "lust"] },
        { emoji: "❤️‍🩹", name: "mending heart", keywords: ["healing", "recovery", "heart"] },
        { emoji: "💕", name: "two hearts", keywords: ["love", "hearts"] },
        { emoji: "💞", name: "revolving hearts", keywords: ["love", "hearts"] },
        { emoji: "💓", name: "beating heart", keywords: ["love", "heart", "heartbeat"] },
        { emoji: "💗", name: "growing heart", keywords: ["love", "heart"] },
        { emoji: "💖", name: "sparkling heart", keywords: ["love", "heart", "sparkle"] },
        { emoji: "💘", name: "heart with arrow", keywords: ["love", "cupid", "valentine"] },
        { emoji: "💝", name: "heart with ribbon", keywords: ["love", "gift", "valentine"] },
        { emoji: "💟", name: "heart decoration", keywords: ["love", "heart"] },

        // Animals
        { emoji: "🐶", name: "dog face", keywords: ["dog", "puppy", "pet"] },
        { emoji: "🐱", name: "cat face", keywords: ["cat", "kitty", "pet"] },
        { emoji: "🐭", name: "mouse face", keywords: ["mouse", "rodent"] },
        { emoji: "🐹", name: "hamster", keywords: ["hamster", "pet", "rodent"] },
        { emoji: "🐰", name: "rabbit face", keywords: ["rabbit", "bunny"] },
        { emoji: "🦊", name: "fox", keywords: ["fox", "animal"] },
        { emoji: "🐻", name: "bear", keywords: ["bear", "animal"] },
        { emoji: "🐼", name: "panda", keywords: ["panda", "bear", "animal"] },
        { emoji: "🐨", name: "koala", keywords: ["koala", "animal", "australia"] },
        { emoji: "🐯", name: "tiger face", keywords: ["tiger", "animal"] },
        { emoji: "🦁", name: "lion", keywords: ["lion", "animal", "king"] },
        { emoji: "🐮", name: "cow face", keywords: ["cow", "animal", "moo"] },
        { emoji: "🐷", name: "pig face", keywords: ["pig", "animal", "oink"] },
        { emoji: "🐸", name: "frog", keywords: ["frog", "animal", "ribbit"] },
        { emoji: "🐵", name: "monkey face", keywords: ["monkey", "animal"] },
        { emoji: "🙈", name: "see-no-evil monkey", keywords: ["monkey", "hide", "shy"] },
        { emoji: "🙉", name: "hear-no-evil monkey", keywords: ["monkey", "ignore"] },
        { emoji: "🙊", name: "speak-no-evil monkey", keywords: ["monkey", "quiet", "secret"] },
        { emoji: "🐔", name: "chicken", keywords: ["chicken", "bird", "animal"] },
        { emoji: "🐧", name: "penguin", keywords: ["penguin", "bird", "animal"] },
        { emoji: "🐦", name: "bird", keywords: ["bird", "animal"] },
        { emoji: "🐤", name: "baby chick", keywords: ["chick", "bird", "baby"] },
        { emoji: "🦆", name: "duck", keywords: ["duck", "bird", "quack"] },
        { emoji: "🦅", name: "eagle", keywords: ["eagle", "bird", "america"] },
        { emoji: "🦉", name: "owl", keywords: ["owl", "bird", "night", "wise"] },
        { emoji: "🦇", name: "bat", keywords: ["bat", "animal", "night", "vampire"] },
        { emoji: "🐺", name: "wolf", keywords: ["wolf", "animal", "howl"] },
        { emoji: "🐗", name: "boar", keywords: ["boar", "pig", "animal"] },
        { emoji: "🐴", name: "horse face", keywords: ["horse", "animal"] },
        { emoji: "🦄", name: "unicorn", keywords: ["unicorn", "magic", "fantasy"] },
        { emoji: "🐝", name: "honeybee", keywords: ["bee", "insect", "honey"] },
        { emoji: "🐛", name: "bug", keywords: ["bug", "insect"] },
        { emoji: "🦋", name: "butterfly", keywords: ["butterfly", "insect", "pretty"] },
        { emoji: "🐌", name: "snail", keywords: ["snail", "slow"] },
        { emoji: "🐙", name: "octopus", keywords: ["octopus", "sea", "ocean"] },
        { emoji: "🦑", name: "squid", keywords: ["squid", "sea", "ocean"] },
        { emoji: "🦐", name: "shrimp", keywords: ["shrimp", "seafood"] },
        { emoji: "🦞", name: "lobster", keywords: ["lobster", "seafood"] },
        { emoji: "🦀", name: "crab", keywords: ["crab", "seafood", "cancer"] },
        { emoji: "🐠", name: "tropical fish", keywords: ["fish", "sea", "ocean"] },
        { emoji: "🐟", name: "fish", keywords: ["fish", "sea", "ocean"] },
        { emoji: "🐬", name: "dolphin", keywords: ["dolphin", "sea", "ocean"] },
        { emoji: "🐳", name: "spouting whale", keywords: ["whale", "sea", "ocean"] },
        { emoji: "🦈", name: "shark", keywords: ["shark", "sea", "ocean", "jaws"] },
        { emoji: "🐊", name: "crocodile", keywords: ["crocodile", "alligator", "reptile"] },
        { emoji: "🐢", name: "turtle", keywords: ["turtle", "slow", "reptile"] },
        { emoji: "🦎", name: "lizard", keywords: ["lizard", "reptile"] },
        { emoji: "🐍", name: "snake", keywords: ["snake", "reptile"] },
        { emoji: "🦖", name: "T-Rex", keywords: ["dinosaur", "trex", "jurassic"] },
        { emoji: "🦕", name: "sauropod", keywords: ["dinosaur", "jurassic"] },

        // Food & Drink
        { emoji: "🍎", name: "red apple", keywords: ["apple", "fruit", "red"] },
        { emoji: "🍐", name: "pear", keywords: ["pear", "fruit"] },
        { emoji: "🍊", name: "tangerine", keywords: ["orange", "fruit", "tangerine"] },
        { emoji: "🍋", name: "lemon", keywords: ["lemon", "fruit", "yellow", "sour"] },
        { emoji: "🍌", name: "banana", keywords: ["banana", "fruit", "yellow"] },
        { emoji: "🍉", name: "watermelon", keywords: ["watermelon", "fruit", "summer"] },
        { emoji: "🍇", name: "grapes", keywords: ["grapes", "fruit", "wine"] },
        { emoji: "🍓", name: "strawberry", keywords: ["strawberry", "fruit", "berry"] },
        { emoji: "🫐", name: "blueberries", keywords: ["blueberry", "fruit", "berry"] },
        { emoji: "🍒", name: "cherries", keywords: ["cherry", "fruit"] },
        { emoji: "🍑", name: "peach", keywords: ["peach", "fruit", "butt"] },
        { emoji: "🥭", name: "mango", keywords: ["mango", "fruit", "tropical"] },
        { emoji: "🍍", name: "pineapple", keywords: ["pineapple", "fruit", "tropical"] },
        { emoji: "🥥", name: "coconut", keywords: ["coconut", "tropical"] },
        { emoji: "🥝", name: "kiwi fruit", keywords: ["kiwi", "fruit"] },
        { emoji: "🍅", name: "tomato", keywords: ["tomato", "vegetable", "red"] },
        { emoji: "🥑", name: "avocado", keywords: ["avocado", "guacamole"] },
        { emoji: "🥦", name: "broccoli", keywords: ["broccoli", "vegetable", "green"] },
        { emoji: "🥒", name: "cucumber", keywords: ["cucumber", "vegetable"] },
        { emoji: "🌶️", name: "hot pepper", keywords: ["pepper", "spicy", "hot", "chili"] },
        { emoji: "🌽", name: "ear of corn", keywords: ["corn", "vegetable"] },
        { emoji: "🥕", name: "carrot", keywords: ["carrot", "vegetable", "orange"] },
        { emoji: "🧄", name: "garlic", keywords: ["garlic", "vegetable"] },
        { emoji: "🧅", name: "onion", keywords: ["onion", "vegetable"] },
        { emoji: "🥔", name: "potato", keywords: ["potato", "vegetable"] },
        { emoji: "🍞", name: "bread", keywords: ["bread", "food", "toast"] },
        { emoji: "🥐", name: "croissant", keywords: ["croissant", "bread", "french"] },
        { emoji: "🥖", name: "baguette bread", keywords: ["baguette", "bread", "french"] },
        { emoji: "🧀", name: "cheese wedge", keywords: ["cheese", "food"] },
        { emoji: "🥚", name: "egg", keywords: ["egg", "food", "breakfast"] },
        { emoji: "🍳", name: "cooking", keywords: ["egg", "frying", "breakfast", "cooking"] },
        { emoji: "🥓", name: "bacon", keywords: ["bacon", "meat", "breakfast"] },
        { emoji: "🥩", name: "cut of meat", keywords: ["steak", "meat", "beef"] },
        { emoji: "🍗", name: "poultry leg", keywords: ["chicken", "meat", "drumstick"] },
        { emoji: "🍖", name: "meat on bone", keywords: ["meat", "bone"] },
        { emoji: "🌭", name: "hot dog", keywords: ["hotdog", "sausage", "food"] },
        { emoji: "🍔", name: "hamburger", keywords: ["burger", "food", "fast food"] },
        { emoji: "🍟", name: "french fries", keywords: ["fries", "food", "fast food"] },
        { emoji: "🍕", name: "pizza", keywords: ["pizza", "food", "italian"] },
        { emoji: "🥪", name: "sandwich", keywords: ["sandwich", "food", "lunch"] },
        { emoji: "🌮", name: "taco", keywords: ["taco", "mexican", "food"] },
        { emoji: "🌯", name: "burrito", keywords: ["burrito", "mexican", "food"] },
        { emoji: "🥗", name: "green salad", keywords: ["salad", "healthy", "food"] },
        { emoji: "🍜", name: "steaming bowl", keywords: ["noodles", "ramen", "soup", "pho"] },
        { emoji: "🍝", name: "spaghetti", keywords: ["pasta", "italian", "food"] },
        { emoji: "🍣", name: "sushi", keywords: ["sushi", "japanese", "food"] },
        { emoji: "🍱", name: "bento box", keywords: ["bento", "japanese", "food", "lunch"] },
        { emoji: "🍛", name: "curry rice", keywords: ["curry", "food", "indian"] },
        { emoji: "🍚", name: "cooked rice", keywords: ["rice", "food", "asian"] },
        { emoji: "🍿", name: "popcorn", keywords: ["popcorn", "movie", "snack"] },
        { emoji: "🍩", name: "doughnut", keywords: ["donut", "dessert", "sweet"] },
        { emoji: "🍪", name: "cookie", keywords: ["cookie", "dessert", "sweet"] },
        { emoji: "🎂", name: "birthday cake", keywords: ["cake", "birthday", "dessert"] },
        { emoji: "🍰", name: "shortcake", keywords: ["cake", "dessert", "sweet"] },
        { emoji: "🧁", name: "cupcake", keywords: ["cupcake", "dessert", "sweet"] },
        { emoji: "🥧", name: "pie", keywords: ["pie", "dessert"] },
        { emoji: "🍫", name: "chocolate bar", keywords: ["chocolate", "candy", "sweet"] },
        { emoji: "🍬", name: "candy", keywords: ["candy", "sweet"] },
        { emoji: "🍭", name: "lollipop", keywords: ["lollipop", "candy", "sweet"] },
        { emoji: "🍦", name: "soft ice cream", keywords: ["ice cream", "dessert", "sweet"] },
        { emoji: "🍧", name: "shaved ice", keywords: ["ice", "dessert"] },
        { emoji: "🍨", name: "ice cream", keywords: ["ice cream", "dessert", "sweet"] },
        { emoji: "☕", name: "hot beverage", keywords: ["coffee", "tea", "drink", "hot"] },
        { emoji: "🍵", name: "teacup without handle", keywords: ["tea", "drink", "green tea"] },
        { emoji: "🧃", name: "beverage box", keywords: ["juice", "drink", "box"] },
        { emoji: "🥤", name: "cup with straw", keywords: ["soda", "drink", "cup"] },
        { emoji: "🧋", name: "bubble tea", keywords: ["boba", "bubble tea", "drink"] },
        { emoji: "🍺", name: "beer mug", keywords: ["beer", "drink", "alcohol"] },
        { emoji: "🍻", name: "clinking beer mugs", keywords: ["beer", "cheers", "drink"] },
        { emoji: "🍷", name: "wine glass", keywords: ["wine", "drink", "alcohol"] },
        { emoji: "🥂", name: "clinking glasses", keywords: ["champagne", "cheers", "celebration"] },
        { emoji: "🍸", name: "cocktail glass", keywords: ["cocktail", "drink", "martini"] },
        { emoji: "🍹", name: "tropical drink", keywords: ["cocktail", "drink", "tropical"] },
        { emoji: "🧊", name: "ice", keywords: ["ice", "cold", "cube"] },

        // Activities & Sports
        { emoji: "⚽", name: "soccer ball", keywords: ["soccer", "football", "sport"] },
        { emoji: "🏀", name: "basketball", keywords: ["basketball", "sport"] },
        { emoji: "🏈", name: "american football", keywords: ["football", "nfl", "sport"] },
        { emoji: "⚾", name: "baseball", keywords: ["baseball", "sport"] },
        { emoji: "🥎", name: "softball", keywords: ["softball", "sport"] },
        { emoji: "🎾", name: "tennis", keywords: ["tennis", "sport"] },
        { emoji: "🏐", name: "volleyball", keywords: ["volleyball", "sport"] },
        { emoji: "🏉", name: "rugby football", keywords: ["rugby", "sport"] },
        { emoji: "🎱", name: "pool 8 ball", keywords: ["billiards", "pool", "8ball"] },
        { emoji: "🏓", name: "ping pong", keywords: ["ping pong", "table tennis", "sport"] },
        { emoji: "🏸", name: "badminton", keywords: ["badminton", "sport"] },
        { emoji: "🏒", name: "ice hockey", keywords: ["hockey", "sport", "ice"] },
        { emoji: "🏑", name: "field hockey", keywords: ["hockey", "sport"] },
        { emoji: "🥅", name: "goal net", keywords: ["goal", "net", "sport"] },
        { emoji: "⛳", name: "flag in hole", keywords: ["golf", "sport"] },
        { emoji: "🏹", name: "bow and arrow", keywords: ["archery", "sport"] },
        { emoji: "🎣", name: "fishing pole", keywords: ["fishing", "sport"] },
        { emoji: "🥊", name: "boxing glove", keywords: ["boxing", "sport", "fight"] },
        { emoji: "🥋", name: "martial arts uniform", keywords: ["karate", "martial arts", "sport"] },
        { emoji: "🎿", name: "skis", keywords: ["skiing", "winter", "sport"] },
        { emoji: "⛷️", name: "skier", keywords: ["skiing", "winter", "sport"] },
        { emoji: "🏂", name: "snowboarder", keywords: ["snowboard", "winter", "sport"] },
        { emoji: "🏄", name: "person surfing", keywords: ["surfing", "sport", "beach"] },
        { emoji: "🏊", name: "person swimming", keywords: ["swimming", "sport", "pool"] },
        { emoji: "🚴", name: "person biking", keywords: ["biking", "cycling", "sport"] },
        { emoji: "🏃", name: "person running", keywords: ["running", "sport", "exercise"] },
        { emoji: "🧘", name: "person in lotus position", keywords: ["yoga", "meditation", "zen"] },
        { emoji: "🎮", name: "video game", keywords: ["game", "gaming", "controller", "play"] },
        { emoji: "🕹️", name: "joystick", keywords: ["game", "arcade", "gaming"] },
        { emoji: "🎲", name: "game die", keywords: ["dice", "game", "luck"] },
        { emoji: "♟️", name: "chess pawn", keywords: ["chess", "game", "strategy"] },
        { emoji: "🎯", name: "direct hit", keywords: ["target", "bullseye", "dart"] },
        { emoji: "🎰", name: "slot machine", keywords: ["casino", "gambling", "jackpot"] },

        // Objects & Symbols
        { emoji: "💻", name: "laptop", keywords: ["computer", "laptop", "work", "code"] },
        { emoji: "🖥️", name: "desktop computer", keywords: ["computer", "desktop", "pc"] },
        { emoji: "⌨️", name: "keyboard", keywords: ["keyboard", "type", "computer"] },
        { emoji: "🖱️", name: "computer mouse", keywords: ["mouse", "click", "computer"] },
        { emoji: "📱", name: "mobile phone", keywords: ["phone", "mobile", "smartphone"] },
        { emoji: "📞", name: "telephone receiver", keywords: ["phone", "call"] },
        { emoji: "💡", name: "light bulb", keywords: ["idea", "light", "bulb"] },
        { emoji: "🔦", name: "flashlight", keywords: ["light", "torch"] },
        { emoji: "🔌", name: "electric plug", keywords: ["plug", "electric", "power"] },
        { emoji: "🔋", name: "battery", keywords: ["battery", "power", "energy"] },
        { emoji: "📷", name: "camera", keywords: ["camera", "photo", "picture"] },
        { emoji: "🎥", name: "movie camera", keywords: ["camera", "movie", "film", "video"] },
        { emoji: "📺", name: "television", keywords: ["tv", "television", "watch"] },
        { emoji: "🎬", name: "clapper board", keywords: ["movie", "film", "action"] },
        { emoji: "🎤", name: "microphone", keywords: ["mic", "sing", "karaoke"] },
        { emoji: "🎧", name: "headphone", keywords: ["headphones", "music", "listen"] },
        { emoji: "🎵", name: "musical note", keywords: ["music", "note", "sound"] },
        { emoji: "🎶", name: "musical notes", keywords: ["music", "notes", "sound"] },
        { emoji: "🎹", name: "musical keyboard", keywords: ["piano", "music", "keyboard"] },
        { emoji: "🎸", name: "guitar", keywords: ["guitar", "music", "rock"] },
        { emoji: "🎺", name: "trumpet", keywords: ["trumpet", "music", "jazz"] },
        { emoji: "🎻", name: "violin", keywords: ["violin", "music", "classical"] },
        { emoji: "🥁", name: "drum", keywords: ["drum", "music", "percussion"] },
        { emoji: "📚", name: "books", keywords: ["books", "read", "library", "study"] },
        { emoji: "📖", name: "open book", keywords: ["book", "read", "study"] },
        { emoji: "📝", name: "memo", keywords: ["note", "write", "memo"] },
        { emoji: "✏️", name: "pencil", keywords: ["pencil", "write", "draw"] },
        { emoji: "🖊️", name: "pen", keywords: ["pen", "write"] },
        { emoji: "📎", name: "paperclip", keywords: ["paperclip", "office", "attach"] },
        { emoji: "📌", name: "pushpin", keywords: ["pin", "location", "mark"] },
        { emoji: "📁", name: "file folder", keywords: ["folder", "file", "directory"] },
        { emoji: "🗂️", name: "card index dividers", keywords: ["files", "organize", "index"] },
        { emoji: "📅", name: "calendar", keywords: ["calendar", "date", "schedule"] },
        { emoji: "🔍", name: "magnifying glass tilted left", keywords: ["search", "find", "zoom"] },
        { emoji: "🔎", name: "magnifying glass tilted right", keywords: ["search", "find", "zoom"] },
        { emoji: "🔒", name: "locked", keywords: ["lock", "secure", "private"] },
        { emoji: "🔓", name: "unlocked", keywords: ["unlock", "open"] },
        { emoji: "🔑", name: "key", keywords: ["key", "lock", "password"] },
        { emoji: "🔔", name: "bell", keywords: ["bell", "notification", "alert"] },
        { emoji: "🔕", name: "bell with slash", keywords: ["mute", "silent", "quiet"] },
        { emoji: "📧", name: "e-mail", keywords: ["email", "mail", "message"] },
        { emoji: "💰", name: "money bag", keywords: ["money", "rich", "dollar"] },
        { emoji: "💵", name: "dollar banknote", keywords: ["money", "dollar", "cash"] },
        { emoji: "💳", name: "credit card", keywords: ["card", "payment", "credit"] },
        { emoji: "⏰", name: "alarm clock", keywords: ["clock", "alarm", "time", "wake"] },
        { emoji: "⌚", name: "watch", keywords: ["watch", "time"] },
        { emoji: "⏱️", name: "stopwatch", keywords: ["timer", "stopwatch", "time"] },
        { emoji: "🧲", name: "magnet", keywords: ["magnet", "attract"] },
        { emoji: "🔧", name: "wrench", keywords: ["tool", "fix", "wrench"] },
        { emoji: "🔨", name: "hammer", keywords: ["tool", "hammer", "build"] },
        { emoji: "🛠️", name: "hammer and wrench", keywords: ["tools", "fix", "settings"] },
        { emoji: "⚙️", name: "gear", keywords: ["settings", "gear", "config"] },
        { emoji: "💎", name: "gem stone", keywords: ["diamond", "gem", "jewel"] },
        { emoji: "🎁", name: "wrapped gift", keywords: ["gift", "present", "birthday"] },
        { emoji: "🎈", name: "balloon", keywords: ["balloon", "party", "birthday"] },
        { emoji: "🎉", name: "party popper", keywords: ["party", "celebration", "confetti"] },
        { emoji: "🎊", name: "confetti ball", keywords: ["party", "celebration", "confetti"] },

        // Symbols
        { emoji: "✅", name: "check mark button", keywords: ["check", "done", "yes", "complete"] },
        { emoji: "❌", name: "cross mark", keywords: ["no", "wrong", "delete", "remove"] },
        { emoji: "❓", name: "question mark", keywords: ["question", "help", "what"] },
        { emoji: "❗", name: "exclamation mark", keywords: ["exclamation", "important", "alert"] },
        { emoji: "⚠️", name: "warning", keywords: ["warning", "caution", "alert"] },
        { emoji: "🚫", name: "prohibited", keywords: ["no", "forbidden", "stop"] },
        { emoji: "⭕", name: "hollow red circle", keywords: ["circle", "ring", "o"] },
        { emoji: "🔴", name: "red circle", keywords: ["red", "circle", "dot"] },
        { emoji: "🟠", name: "orange circle", keywords: ["orange", "circle", "dot"] },
        { emoji: "🟡", name: "yellow circle", keywords: ["yellow", "circle", "dot"] },
        { emoji: "🟢", name: "green circle", keywords: ["green", "circle", "dot"] },
        { emoji: "🔵", name: "blue circle", keywords: ["blue", "circle", "dot"] },
        { emoji: "🟣", name: "purple circle", keywords: ["purple", "circle", "dot"] },
        { emoji: "⚫", name: "black circle", keywords: ["black", "circle", "dot"] },
        { emoji: "⚪", name: "white circle", keywords: ["white", "circle", "dot"] },
        { emoji: "🔺", name: "red triangle pointed up", keywords: ["triangle", "up", "red"] },
        { emoji: "🔻", name: "red triangle pointed down", keywords: ["triangle", "down", "red"] },
        { emoji: "🔶", name: "large orange diamond", keywords: ["diamond", "orange"] },
        { emoji: "🔷", name: "large blue diamond", keywords: ["diamond", "blue"] },
        { emoji: "💯", name: "hundred points", keywords: ["100", "perfect", "score"] },
        { emoji: "🔥", name: "fire", keywords: ["fire", "hot", "lit", "flame"] },
        { emoji: "✨", name: "sparkles", keywords: ["sparkle", "shine", "magic", "stars"] },
        { emoji: "⭐", name: "star", keywords: ["star", "favorite"] },
        { emoji: "🌟", name: "glowing star", keywords: ["star", "glow", "shine"] },
        { emoji: "💫", name: "dizzy", keywords: ["star", "dizzy", "shooting"] },
        { emoji: "⚡", name: "high voltage", keywords: ["lightning", "electric", "power", "fast"] },
        { emoji: "💥", name: "collision", keywords: ["boom", "explosion", "crash"] },
        { emoji: "🌈", name: "rainbow", keywords: ["rainbow", "colors", "pride"] },
        { emoji: "☀️", name: "sun", keywords: ["sun", "sunny", "weather", "hot"] },
        { emoji: "🌙", name: "crescent moon", keywords: ["moon", "night", "sleep"] },
        { emoji: "⛅", name: "sun behind cloud", keywords: ["weather", "cloudy", "partly"] },
        { emoji: "☁️", name: "cloud", keywords: ["cloud", "weather"] },
        { emoji: "🌧️", name: "cloud with rain", keywords: ["rain", "weather", "rainy"] },
        { emoji: "⛈️", name: "cloud with lightning and rain", keywords: ["storm", "thunder", "weather"] },
        { emoji: "❄️", name: "snowflake", keywords: ["snow", "winter", "cold", "ice"] },
        { emoji: "🌊", name: "water wave", keywords: ["wave", "ocean", "sea", "water"] },
        { emoji: "💧", name: "droplet", keywords: ["water", "drop", "tear"] },
        { emoji: "🌍", name: "globe showing Europe-Africa", keywords: ["earth", "world", "globe", "planet"] },
        { emoji: "🌎", name: "globe showing Americas", keywords: ["earth", "world", "globe", "planet"] },
        { emoji: "🌏", name: "globe showing Asia-Australia", keywords: ["earth", "world", "globe", "planet"] },
        { emoji: "🚀", name: "rocket", keywords: ["rocket", "space", "launch", "fast"] },
        { emoji: "✈️", name: "airplane", keywords: ["plane", "travel", "flight"] },
        { emoji: "🚗", name: "automobile", keywords: ["car", "vehicle", "drive"] },
        { emoji: "🚕", name: "taxi", keywords: ["taxi", "cab", "car"] },
        { emoji: "🚌", name: "bus", keywords: ["bus", "transport"] },
        { emoji: "🚲", name: "bicycle", keywords: ["bike", "bicycle", "cycle"] },
        { emoji: "🏠", name: "house", keywords: ["house", "home", "building"] },
        { emoji: "🏢", name: "office building", keywords: ["office", "building", "work"] },
        { emoji: "🏥", name: "hospital", keywords: ["hospital", "medical", "health"] },
        { emoji: "🏫", name: "school", keywords: ["school", "education", "building"] },
        { emoji: "⬆️", name: "up arrow", keywords: ["up", "arrow", "north"] },
        { emoji: "⬇️", name: "down arrow", keywords: ["down", "arrow", "south"] },
        { emoji: "⬅️", name: "left arrow", keywords: ["left", "arrow", "west"] },
        { emoji: "➡️", name: "right arrow", keywords: ["right", "arrow", "east"] },
        { emoji: "↩️", name: "right arrow curving left", keywords: ["back", "return", "undo"] },
        { emoji: "↪️", name: "left arrow curving right", keywords: ["forward", "redo"] },
        { emoji: "🔄", name: "counterclockwise arrows button", keywords: ["refresh", "reload", "sync"] },
        { emoji: "➕", name: "plus", keywords: ["plus", "add", "positive"] },
        { emoji: "➖", name: "minus", keywords: ["minus", "subtract", "negative"] },
        { emoji: "✖️", name: "multiply", keywords: ["multiply", "times", "x"] },
        { emoji: "➗", name: "divide", keywords: ["divide", "division"] },
        { emoji: "♾️", name: "infinity", keywords: ["infinity", "forever", "endless"] },
        { emoji: "💲", name: "heavy dollar sign", keywords: ["dollar", "money", "price"] },
        { emoji: "©️", name: "copyright", keywords: ["copyright", "legal"] },
        { emoji: "®️", name: "registered", keywords: ["registered", "trademark"] },
        { emoji: "™️", name: "trade mark", keywords: ["trademark", "tm"] },
        { emoji: "🔗", name: "link", keywords: ["link", "chain", "url"] },
        { emoji: "ℹ️", name: "information", keywords: ["info", "information", "help"] },
        { emoji: "🆕", name: "NEW button", keywords: ["new", "fresh"] },
        { emoji: "🆓", name: "FREE button", keywords: ["free", "gratis"] },
        { emoji: "🆗", name: "OK button", keywords: ["ok", "okay", "good"] },
        { emoji: "🆘", name: "SOS button", keywords: ["sos", "help", "emergency"] }
    ]

    // Search emojis
    function search(query, isPrefixSearch) {
        let queryLower = query.toLowerCase().trim()

        // If no query and prefix search, show popular emojis
        if (!queryLower && isPrefixSearch) {
            return emojis.slice(0, maxResults).map(createResult)
        }

        if (!queryLower) return []

        // Filter by name or keywords
        let filtered = emojis.filter(e => {
            if (e.name.includes(queryLower)) return true
            return e.keywords.some(k => k.includes(queryLower))
        })

        // Sort by relevance (name starts with query first)
        filtered.sort((a, b) => {
            let aStarts = a.name.startsWith(queryLower)
            let bStarts = b.name.startsWith(queryLower)
            if (aStarts && !bStarts) return -1
            if (!aStarts && bStarts) return 1

            // Check keyword match
            let aKeyword = a.keywords.some(k => k.startsWith(queryLower))
            let bKeyword = b.keywords.some(k => k.startsWith(queryLower))
            if (aKeyword && !bKeyword) return -1
            if (!aKeyword && bKeyword) return 1

            return a.name.localeCompare(b.name)
        })

        return filtered.slice(0, maxResults).map(createResult)
    }

    function createResult(e) {
        return {
            type: "emoji",
            emoji: e.emoji,
            title: e.name,
            data: e,
            action: function() {
                copyToClipboard(e.emoji)
            }
        }
    }

    // Copy to clipboard
    Process {
        id: copyProcess
        command: ["wl-copy", ""]
    }

    function copyToClipboard(text) {
        copyProcess.command = ["wl-copy", text]
        copyProcess.running = true
    }
}
